...
const packagesData = [
	{
		...
		description: 'Perfect for small businesses wanting a professional online presence without ongoing fees.',
...
const addonsData = [
	{
		...
		slug: 'monthly-seo',
		description: 'Ongoing SEO optimization including keyword research, content updates, and link building.',
		price: '350.00',
		pricingType: 'monthly',
		availablePackages: [],
		displayOrder: 1,
...
Our approach combines stunning visual design with proven conversion optimization techniques, ensuring your new website works as hard as you do.`,
	...
	proposedPages: [
		{ name: 'Home', description: 'High-converting landing page with emergency CTA', features: ['24/7 emergency banner', 'Click-to-call button'] },
		{ name: 'Services', description: 'Comprehensive service pages', features: ['Emergency plumbing', 'Hot water systems'] },
		{ name: 'Contact', description: 'Multiple contact options', features: ['Online booking form', 'Live chat widget'] }
	],
	timeline: [
		{ week: 'Week 1-2', title: 'Discovery & Strategy', description: 'Deep dive into your business...' },
	...
	consultationPainPoints: {
		primary_challenges: ['Not getting enough leads from website', 'Competitors ranking higher on Google'],
...
async function seedFullDemo() {
	...
	try {
		...
		if (existingAgency) {
			...
			await db
				.update(users)
				.set({ defaultAgencyId: IDS.AGENCY })
		...
		if (!existingContract) {
			await db.insert(contracts).values({
				...
				viewCount: 0,
				visibleFields: ['services', 'commencementDate', 'completionDate', 'price', 'paymentTerms', 'specialConditions'],
				includedScheduleIds: [],
				createdBy: user.id,
				createdAt: new Date(),
				updatedAt: new Date()
