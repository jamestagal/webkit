...
const consultationData = {
	...
	painPoints: {
		...
		urgency_level: 'high',
		impact_assessment: 'Estimating $18,000-$27,000 in lost monthly revenue due to website issues. Lost 3 large commercial contracts ($15,000+) in past 6 months to competitors with better websites. Staff spending 2+ hours daily answering questions that should be on website.',
		...
		competitor_comparison: 'Main competitors (Brisbane Plumbing Co, Jet Plumbing) have modern, fast-loading sites with online booking, prominent reviews, and rank on page 1 for key terms. We are stuck on page 2-3.',
...
async function seedConsultation() {
	...
	try {
		...
		if (existingConsultation) {
			...
			console.log(`   Client: ${(existingConsultation.contactInfo as Record<string, string>).business_name || 'Unknown'}`);
		...
		console.log(`  npx tsx scripts/seed-proposal.ts ${agencySlug} --consultation ${CONSULTATION_ID}`);
		console.log('\n');
