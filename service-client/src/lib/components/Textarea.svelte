...
<div class="flex w-full flex-col gap-2">
	...
	{/if}
	<div
		class="border-primary-4 [&>textarea]:placeholder:text-secondary-4 relative flex w-full overflow-hidden rounded-xl border
    p-1 focus-within:outline-2 [&>textarea]:w-full"
	>
		<textarea
			class="min-h-24 w-full p-1 outline-none"
			{id}
			{name}
			{placeholder}
			{...restProps}
		...
		{#if showLimit}
			<div
				class="absolute right-3 bottom-3 text-xs {isOverLimit ? 'text-red-500' : 'text-zinc-500 '}"
			>
