<script lang="ts">
  ...
  function getStepLabel(step: any, stepIndex: number): string {
    ...
    switch (status) {
      case 'completed': return ` ${step.title}`;
      case 'current': return `� ${step.title}`;
...
<div class="w-full">
    ...
    <div class="flex justify-between items-center mb-2">
      {#if showStepCount}
        <div class="flex items-center space-x-2">
          <span class="{textSizeClasses[size]} font-medium text-gray-700">
            Progress
          </span>
          {#if currentStepInfo()}
  ...
  <div class="relative">
    <div class="w-full bg-gray-200 rounded-full {sizeClasses[size]} overflow-hidden">
      <div
        class="{variantClasses[variant]} {sizeClasses[size]} rounded-full {animationClass} relative overflow-hidden"
      ...
      >
        ...
        {#if animate && progress() > 0}
          <div class="absolute inset-0 bg-white opacity-20 -skew-x-12 transform translate-x-full animate-pulse"></div>
        {/if}
      </div>
    </div>
      ...
      <div class="absolute top-0 left-0 w-full h-full flex justify-between items-center px-1">
        {#each steps as step, index}
          {#if index > 0 && index < totalSteps() - 1}
            ...
            <div
              class="w-1 {sizeClasses[size]} rounded-full transform -translate-x-1/2 {
  ...
  {#if showLabels}
    <div class="mt-2">
      <p class="{textSizeClasses[size]} text-gray-600 text-center">
        {statusMessage()}
      </p>
    </div>
  {/if}
    ...
    <div class="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-2">
      {#each steps as step, index}
        ...
        <div class="flex items-center space-x-2 {textSizeClasses[size]}">
          <span class="flex-shrink-0 w-4 h-4 rounded-full flex items-center justify-center text-xs {
            status === 'completed'
