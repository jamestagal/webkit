...
<div
	tabindex="0"
	role="button"
	class="fixed z-50 {positionClass[position]
		?.class} group flex items-center -space-y-12 hover:space-y-4"
...
>
	...
	{#each toasts as singleToast, i (singleToast.id)}
		...
		<div
			...
			class="flex min-h-16 w-fit min-w-xs items-center rounded-xl p-3 text-sm backdrop-blur-sm transition-all duration-300 group-hover:!scale-100 {styles[
				singleToast.type
			]}"
			style="z-index: {100 + singleToast.id}; scale: {1 - i / 10}; box-shadow: 0 {direction ===
			'top'
				? '4px'
				: '-4px'} 14px rgba(0, 0, 0, 0.2);"
		>
			<div class="flex w-full flex-col items-start gap-1">
				{#if singleToast.title}
					<p class="text-xs font-bold uppercase">{singleToast.type}</p>
				{/if}
			...
			{#if singleToast.closable}
				<button
					type="button"
					class="bg-secondary text-main ml-1 flex h-5 w-5 flex-shrink-0 items-center justify-center rounded-full p-1"
					onclick={() => {
						toast.remove(singleToast.id);
					}}
