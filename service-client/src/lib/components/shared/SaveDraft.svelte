...
<div class={positionClasses[position]}>
	<div
		class="flex items-center space-x-3 {position === 'inline'
			? ''
			: 'rounded-lg border bg-white px-4 py-2 shadow-lg'} {compact ? 'text-sm' : 'text-base'}"
	>
		<!-- Status Indicator -->
		{#if showStatus}
			<div class="flex items-center space-x-2">
				...
				<div class="flex-shrink-0">
					{#if saveStatusInfo.icon === "saving"}
						<svg
							class="h-4 w-4 animate-spin {saveStatusInfo.className}"
							fill="none"
							viewBox="0 0 24 24"
						>
							<circle
					...
					{:else if saveStatusInfo.icon === "success"}
						<svg class="h-4 w-4 {saveStatusInfo.className}" fill="currentColor" viewBox="0 0 20 20">
							<path
								fill-rule="evenodd"
					...
					{:else if saveStatusInfo.icon === "saved"}
						<svg
							class="h-4 w-4 {saveStatusInfo.className}"
							fill="none"
							stroke="currentColor"
							viewBox="0 0 24 24"
						>
		...
		{#if showButton}
			<button
				...
				class="inline-flex items-center rounded-md border border-transparent bg-indigo-600 px-3 py-1.5 text-xs font-medium text-white transition-colors duration-200 hover:bg-indigo-700 disabled:cursor-not-allowed disabled:opacity-50 {compact
					? 'px-2 py-1 text-xs'
					: ''}"
	...
	{#if !showStatus && (isAutoSaving || showSuccessAnimation)}
		<div
			class="absolute -top-1 -right-1 h-3 w-3 rounded-full {isAutoSaving
	...
	{#if lastSaveStatus === "error"}
		<div
			class="absolute top-full right-0 mt-2 min-w-64 rounded-lg border border-red-200 bg-red-50 p-3 shadow-lg"
		>
			<div class="flex">
				<div class="flex-shrink-0">
				...
				<div class="ml-3">
					...
					<div class="mt-3">
						<button
							...
							class="rounded-md bg-red-100 px-2 py-1 text-sm text-red-800 hover:bg-red-200"
						>
							Retry Save
						</button>
					</div>
...
{#if showSuccessAnimation && position !== "inline"}
	<div
		class="fixed top-4 right-4 z-50 transform rounded-lg bg-green-500 px-4 py-2 text-white shadow-lg transition-all duration-500 {showSuccessAnimation
...
{#if position === "inline" && autoSave}
	<div class="mt-2 text-xs text-gray-500">Changes are automatically saved as you type</div>
{/if}
