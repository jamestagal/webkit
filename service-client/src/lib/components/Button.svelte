<script>
    ...
    const commonStyles = 'cursor-pointer rounded-xl scale-100 active:scale-95 transition-all duration-100 font-medium disabled:opacity-30 disabled:pointer-events-none';
    const buttonStyles = {
...
{:else}
    <button type={type} class={styles} {disabled} {...restProps}>
        <span class="relative flex flex-row items-center justify-center">
            {#if isLoading}
                <svg 
                    xmlns="http://www.w3.org/2000/svg" 
