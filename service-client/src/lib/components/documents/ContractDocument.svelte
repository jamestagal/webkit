<script lang="ts">
	/**
	 * ContractDocument — self-contained contract renderer.
	 *
	 * Contract branding uses only the generic `EffectiveBranding` (no contract-
	 * specific slots today). The wrapper injects the 4 generic `--brand-*`
	 * variables so the wrapper API stays consistent across document types;
	 * the current contract template has no inline color consumers (only
	 * logo via `branding.logoUrl`), so the vars are effectively a forward-
	 * compatible hook for future per-site brand theming.
	 *
	 * All state (signature form, success handling) lives inside the component.
	 *
	 * `previewMode` hides the client signature form + the "Signature hidden
	 * in preview mode" placeholder.
	 */
	...
	import type { PageData, ActionData } from '../../../routes/c/[slug]/$types';
