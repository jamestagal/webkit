<script lang="ts">
	/**
	 * QuotationDocument — self-contained quotation renderer.
	 *
	 * Lightweight sibling of ProposalDocument. Quotation branding uses only
	 * the generic `EffectiveBranding` (logo + primary + secondary + accent +
	 * accent-gradient) — there are no proposal-specific slots (cover/section
	 * heading/CTA/footer). The wrapper injects the 4 generic `--brand-*`
	 * variables; consumers read them via `var(...)`.
	 *
	 * All state lives inside the component: action-result banners (accept /
	 * decline success), PDF download flow, print handler, status badge logic,
	 * acceptance form state, decline form state. Parent page is a thin SEO +
	 * invocation shell.
	 *
	 * `previewMode` hides every interactive affordance (Download / Print
	 * actions bar, Accept / Decline forms) so the branding preview iframe
	 * shows a clean read-only rendering.
	 */
	import {
		CheckCircle,
		AlertCircle,
	...
	import type { PageData, ActionData } from '../../../routes/q/[slug]/$types';
	...
	// Reactive top-level derivations so any loader refresh (e.g. after a form
	// action) propagates into the view.
...
>
	<div class="min-h-screen bg-base-200 py-8">
		<div class="container mx-auto px-4 max-w-4xl">
			...
			<!-- Actions Bar (hidden in print + preview) -->
			{#if !previewMode}
					...
					<div class="flex flex-col sm:flex-row justify-between gap-4 pb-6 border-b border-base-300">
						<div>
							{#if branding.logoUrl}
								...
								/>
							{:else}
								<h2 class="text-2xl font-bold" style="color: var(--brand-primary)">
									{agency?.name || 'Agency'}
								</h2>
							{/if}
							{#if profile?.tagline}
			...
			<!-- Contact Info -->
			{#if !previewMode && (agency?.email || agency?.phone)}
