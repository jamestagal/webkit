<script lang="ts">
	/**
	 * InvoiceDocument — self-contained invoice renderer.
	 *
	 * Generic EffectiveBranding only (logo + primary/secondary/accent +
	 * accent-gradient). Wrapper injects the 4 generic `--brand-*` vars;
	 * only the fallback "agency name" h2 (when no logo is set) currently
	 * consumes `--brand-primary`.
	 *
	 * `previewMode` hides the Download / Print action bar, the Pay Now
	 * button, and the Powered-by footer — leaving a clean read-only
	 * rendering for the branding preview iframe.
	 */
	...
	import type { PageData } from '../../../routes/i/[slug]/$types';
