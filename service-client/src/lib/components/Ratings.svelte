...
<script>
	import { UserCircle as UserRound } from "lucide-svelte";
...
<div class="flex flex-row flex-wrap items-center justify-center gap-4">
	...
	{#if count > 0}
		<div class="flex -space-x-2">
			{#each Array(Math.min(maxAvatarsToShow + 1, 5)) as _, i}
				<div
					class="card card-ring text-secondary-3 flex h-12 w-12 items-center justify-center !rounded-full"
				>
	...
	<div
		class="shimmer-container relative flex flex-col gap-1 overflow-hidden"
	...
	>
		<div class="flex items-center gap-0.5">
			...
			<span
				class="text-secondary-3 ml-1.5 text-sm font-bold"
