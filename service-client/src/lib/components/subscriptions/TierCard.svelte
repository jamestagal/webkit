<script>
    ...
    let { product, intent = null, userSubscription = null } = $props();
    
    // Check if this product is the user's current subscription
    let isCurrentPlan = $derived(() => {
        if (!userSubscription || !userSubscription.productId) return false;
        return userSubscription.productId._id === product.id || 
               userSubscription.productId.id === product.id ||
               userSubscription.polarProductId === product.id;
    });
    ...
    // Handle checkout with intent parameter
    async function handleCheckout() {
        try {
            // Always store intent in sessionStorage for success page
            if (intent) {
                sessionStorage.setItem('checkout_intent', intent);
                console.log('Stored checkout intent in sessionStorage:', intent);
            }
            ...
            // Also add metadata to pass intent through Polar checkout
            if (intent) {
            ...
            const result = await authClient.checkout(checkoutData);
            
            // The authClient.checkout should redirect automatically
            if (result.url) {
  ...
  <div class="card space-y-8  max-w-xs bg-white {special ? '!border-transparent shadow-xl' : 'card-ring'} rounded-2xl p-6">
        <div class="space-y-6">
            ...
            <div class="flex flex-col gap-2">
                <button onclick={handleCheckout} class="button center cta">
