...
<Modal bind:isOpen>
	<div class="flex flex-col gap-4 p-6">
		<div class="text-center">
			<h3 class="text-secondary mb-2 text-lg font-medium">Crop Image</h3>
			<p class="text-secondary/70 text-sm">
				Drag to pan • Scroll to zoom • Pinch to zoom • Arrow keys to pan • +/- to zoom
			</p>
		</div>
		...
		<div class="flex justify-center">
			<div
				...
				class="bg-primary border-secondary focus:ring-secondary relative cursor-grab overflow-hidden rounded-lg border-2 shadow-lg focus:ring-2 focus:outline-none"
		...
		<!-- Image info -->
		<div class="text-secondary/60 space-y-1 text-center text-xs">
			<div>
				Image: {imageDimensions.naturalWidth} × {imageDimensions.naturalHeight}px • Zoom: {Math.round(
					imageScale * 100,
				)}% • Crop: {cropSize.width} × {cropSize.height}px • Ratio: {aspectRatio.label}
			</div>
			{#if getCurrentSelection()}
				{@const selection = getCurrentSelection()}
				<div class="bg-primary-2 text-secondary rounded px-2 py-1 font-mono">
		...
		<div class="flex justify-end gap-2">
			...
			<Button onclick={handleSelect} variant="action">Select</Button>
		</div>
	</div>
</Modal>
