...
<!-- Content -->
<div class="grid grid-cols-1 grid-rows-1 w-full overflow-x-hidden">
    {#key activeIndex}
        <div class="col-start-1 row-start-1 flex flex-col gap-4 w-full"
            in:fly={{ x: inDir, duration: 500, delay: 200 }}
            out:fly={{ x: outDir, duration: 300 }}>
            {#if children}
                {@render children({ activeIndex, prevIndex })}
            {:else}
                <p class="text-xs text-fade italic">Content missing for index: {activeIndex}</p>
            {/if}
        </div>
    {/key}
</div>