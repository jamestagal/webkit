...
<div class="space-y-6">
  ...
  <div>
    ...
    <p class="mt-1 text-sm text-gray-600">
      Add any additional information, requirements, or questions that weren't covered in the previous sections.
    </p>
  </div>
      ...
      <div class="grid grid-cols-1 gap-2 sm:grid-cols-2 lg:grid-cols-3">
        {#each suggestionTemplates as template}
          <button
            ...
            disabled={disabled}
            class="text-left p-3 border border-gray-300 rounded-lg hover:border-indigo-300 hover:bg-indigo-50 disabled:opacity-50 disabled:cursor-not-allowed transition-colors duration-200"
          >
  ...
  <div>
    ...
    <div class="relative">
      <Textarea
        ...
        rows={6}
        disabled={disabled}
        error={isOverLimit ? `Exceeded character limit by ${Math.abs(charactersRemaining)} characters` : ''}
        class="resize-none"
