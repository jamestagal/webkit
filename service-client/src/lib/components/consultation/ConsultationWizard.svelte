<script lang="ts">
  ...
  let goalsObjectivesData = $derived.by(() => consultationStore.formState.data.goals_objectives || {});
  ...
  function getCurrentFormErrors() {
    ...
    switch (currentStep()) {
      case 0: return errors['contact_info'] || [];
      case 1: return errors['business_context'] || [];
      case 2: return errors['pain_points'] || [];
      case 3: return errors['goals_objectives'] || [];
      default: return [];
    }
  }
</script>
  ...
  <div class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
    <div class="bg-white rounded-lg p-6 max-w-md w-full mx-4">
      ...
      <div class="flex space-x-3">
        <Button variant="primary" onclick={() => { handleSave(); showExitConfirmation = false; }}>
          Save & Exit
        </Button>
        <Button variant="secondary" onclick={() => showExitConfirmation = false}>
          Continue Editing
        </Button>
        <Button variant="danger" onclick={() => { consultationStore.reset(); showExitConfirmation = false; }}>
          Exit Without Saving
        </Button>
      </div>
    </div>
