<script>
	...
	let displayPrice = $derived.by(() => {
		const monthlyPrice = product.prices?.find(
			(p) => p.amountType === "fixed" && p.interval === "month",
		);
		const yearlyPrice = product.prices?.find(
			(p) => p.amountType === "fixed" && p.interval === "year",
		);
		const oneTimePrice = product.prices?.find(
			(p) => p.amountType === "fixed" && p.interval === "one_time",
		);
...
<div class={special ? `animate-border rounded-xl` : ""}>
	<div
		class="card max-w-xs space-y-8 bg-white {special
			? '!border-transparent shadow-xl'
			: 'card-ring'} rounded-2xl p-6"
	>
		<div class="space-y-6">
			{#if product.title}
			...
			<p class="text-secondary-4 text-sm break-words whitespace-break-spaces">
				{product.description}
			</p>
			<div class="flex flex-col gap-2">
				<a href={product.url || "/settings/billing"} class="button center cta">
					Get {product.name}
					<ArrowRight size={18} strokeWidth={2.5} class="ml-1" />
				</a>
		...
		{#if included.length > 0}
			<div class="border-primary-2 space-y-4 border-t pt-4">
		...
		{#if benefits.length > 0}
			<div class="bg-primary rounded-lg p-4">
				<h4 class="mb-2 font-medium text-zinc-900">After purchase, you'll get:</h4>
