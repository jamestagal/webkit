...
<div class="grid grid-cols-6 gap-2 w-full max-w-md">
    {#each pinCode as digit, i}
        ...
        <input
            ...
            disabled={isLoading}
            autocomplete="off"
            class="w-full max-w-16 aspect-square !bg-main text-center text-xl border border-primary-3 focus:outline-secondary-3
            rounded-xl {isLoading ? 'opacity-50 bg-primary-2' : ''}"
        />
    {/each}
</div>
