...
describe('API Integration Tests', () => {
  ...
  describe('HttpClient', () => {
    test('should make successful GET request', async () => {
      ...
      const result = await httpClient.request<typeof mockData>('/api/test');
    ...
    test('should handle POST request with JSON data', async () => {
      ...
      const result = await httpClient.request('/api/users', {
        method: 'POST',
        json: postData,
      });
    ...
    test('should handle FormData requests', async () => {
      ...
      const result = await httpClient.request('/api/upload', {
        method: 'POST',
        form: formData,
      });
    ...
    test('should include authorization header when token provided', async () => {
      ...
      await httpClient.request('/api/protected', {
        method: 'GET',
        token: 'test-token',
      });
    ...
    test('should handle 4xx client errors', async () => {
      const errorResponse = new Response(
        JSON.stringify({ message: 'Bad Request', error: 'validation_failed' }),
        {
          status: 400,
      ...
      const result = await httpClient.request('/api/invalid');
    ...
    test('should retry on 5xx server errors', async () => {
      ...
      const result = await httpClient.request('/api/retry-test', {
        retries: 1,
    ...
    test('should validate response with Zod schema', async () => {
      ...
      const testSchema = ConsultationSchema;
      const result = await httpClient.request('/api/consultation/1', {
        validateWith: testSchema,
        validationContext: 'consultation-get',
      });

      // This should fail validation since mockData doesn't match ConsultationSchema
      expect(result.success).toBe(false);
      if (!result.success) {
        expect(result.code).toBe(400);
  ...
  describe('ConsultationApiService', () => {
    ...
    test('should create consultation', async () => {
      ...
      const result = await consultationApiService.createConsultation('valid-token', consultationInput);
  ...
  describe('Error Handling & Edge Cases', () => {
    test('should handle network errors', async () => {
      ...
      const result = await httpClient.request('/api/test', { retries: 0 });
    ...
    test('should handle timeout errors', async () => {
      ...
      const result = await httpClient.request('/api/slow', {
    ...
    test('should handle invalid JSON responses', async () => {
      ...
      const result = await httpClient.request('/api/invalid-json');
    ...
    test('should handle non-JSON responses', async () => {
      const mockResponse = new Response('<html>Not JSON</html>', {
        status: 200,
      ...
      const result = await httpClient.request('/api/html-response');

      expect(result.success).toBe(false);
      if (!result.success) {
        expect(result.message).toContain('Expected JSON response');
      }
    });
  });
  ...
  describe('Integration Workflow', () => {
    test('should complete full consultation workflow', async () => {
      ...
      const consultationResponse = new Response(JSON.stringify({
        id: 'consultation-123',
