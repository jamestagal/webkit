...
describe("Consultation Form Components", () => {
	...
	describe("GoalsObjectives", () => {
		...
		it("should validate required fields", async () => {
			...
			await waitFor(() => {
				expect(
					screen.getByText(/please add at least one primary goal and select a budget range/i),
				).toBeInTheDocument();
			});
		});
	...
	describe("Form Validation Integration", () => {
		...
		it("should validate JSON format for social media", async () => {
			...
			// Valid JSON
			await fireEvent.input(socialMediaInput, {
				target: { value: '{"linkedin": "https://linkedin.com"}' },
			});
			await waitFor(() => {
