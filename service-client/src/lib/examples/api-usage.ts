...
/**
 * Example 3: Direct API Usage with Consultation Service
 */
export async function directApiExample() {
	try {
		...
		// Make a request using the consultation service
		console.log("Making API request via consultation service...");
		const response = await consultationApiService.getConsultation(token, "consultation-123");
...
export async function draftManagementExample(consultationId: string) {
	try {
		...
		const createDraftResult = await consultationApiService.createDraft(
			token,
			consultationId,
			draftData,
		);
		...
		const saveDraftResult = await consultationApiService.saveDraft(
			token,
			consultationId,
			updatedDraftData,
		);
