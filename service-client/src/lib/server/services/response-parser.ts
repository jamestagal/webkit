 ...
 */
export function parseAIResponse(rawText: string, options: ParseOptions = {}): AIProposalOutput {
	if (!rawText || rawText.trim().length === 0) {
		throw new AIServiceError("Empty response from AI", AIErrorCode.RESPONSE_EMPTY, true);
	}
...
function normalizePriority(value: unknown): "essential" | "recommended" | "optional" {
	...
	const lower = value.toLowerCase();
	if (lower.includes("essential") || lower.includes("required") || lower.includes("must"))
		return "essential";
