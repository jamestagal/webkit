...
export function validateSchema<T>(
	...
	try {
		...
		const errors = (zodError as any).errors.map((err: any) => {
 ...
 */
export function safeValidate<T>(data: unknown, schema: ZodSchema<T>, context?: string): Safe<T> {
...
export class ValidationChain<T> {
	...
	constructor(
		private initialSchema: ZodSchema<T>,
	 ...
	 */
	addRule(rule: (data: T) => boolean, errorMessage: string): ValidationChain<T> {
