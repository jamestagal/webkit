/**
 * Questionnaire PDF Generation API Endpoint
 *
 * GET /api/questionnaires/[questionnaireId]/pdf - Download questionnaire as PDF
 *
 * Uses Gotenberg for HTML-to-PDF conversion with professional template.
 * Requires agency membership and questionnaire access permission.
 */

import { json } from '@sveltejs/kit';
import type { RequestHandler } from './$types';
import { db } from '$lib/server/db';
import { questionnaireResponses, agencies, agencyProfiles } from '$lib/server/schema';
import { eq } from 'drizzle-orm';
import { generateQuestionnairePdfHtml } from '$lib/templates/questionnaire-pdf';
...
export const GET: RequestHandler = async ({ params }) => {
	...
	if (!questionnaireId) {
		return json({ error: 'Questionnaire ID is required' }, { status: 400 });
	}
	...
	try {
		...
		// Generate filename from client business name
		const clientName = questionnaire.clientBusinessName
			? questionnaire.clientBusinessName
					.toLowerCase()
					.replace(/[^a-z0-9]+/g, '-')
					.replace(/^-|-$/g, '')
			: 'questionnaire';
		const dateStr = new Date().toISOString().split('T')[0];
		const filename = `questionnaire-${clientName}-${dateStr}.pdf`;
