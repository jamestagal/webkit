<script lang="ts">
	...
	// Editable fields — initial snapshot via untrack();
	// $effect below keeps them synced from server data.
	let editTitle = $state(untrack(() => data.copy.title));
	let editContent = $state(untrack(() => data.copy.content));
